...
public final class NodeImpl extends Node {
	 ...
	 * nodeID appended. The length of the nodeID depends on the number of bytes that
	 * are displayed when the ID is shown in Hex-Representation. See documentation
	 * of {@link ID}. E.g. de.uniba.wiai.lspi.chord.service.impl.NodeImpl.FF FF FF
	...
	/**
	 * Executor that executes insertion and removal of entries on successors of this
	 * node.
	 */
	...
	/**
	 * Creates that part of the local node which answers remote requests by other
	 * nodes. Sole constructor, is invoked by ChordImpl only.
	 * 
	 * @param impl
	...
	/**
	 * Makes this endpoint accept entries by other nodes. Is invoked by ChordImpl
	 * only.
	 */
	...
	public final RefsAndEntries notifyAndCopyEntries(Node potentialPredecessor) throws CommunicationException {
		...
		try {
			...
			RefsAndEntries entries = new RefsAndEntries(this.notify(potentialPredecessor), copiedEntries);
	...
	public final void insertEntry(Entry toInsert) throws CommunicationException {
		...
		if ((this.references.getPredecessor() == null)
				|| (!toInsert.getId().isInInterval(this.references.getPredecessor().getNodeID(), this.nodeID)
						&& !toInsert.getId().equals(this.nodeID))) { // Modified by Daniel Shen, for the case that
																		// the key is equal to the local ID
			// TODO by daniel shen
			if(toInsert == null) {
				System.out.println("toInsert is null");
			} else if(references == null) {
				System.out.println("references is null");
			} else if(references.getPredecessor() == null) {
				System.out.println("getPredecessor() is null");
			}
	...
	public final void removeEntry(Entry entryToRemove) throws CommunicationException {
		...
		if (this.references.getPredecessor() != null
				&& (!entryToRemove.getId().isInInterval(this.references.getPredecessor().getNodeID(), this.nodeID)
						&& !entryToRemove.getId().equals(this.nodeID))) {// Modified by Daniel Shen, for the case
																			// that the key is equal to the local ID
	...
	public final void removeReplicas(ID sendingNodeID, Set<Entry> replicasToRemove) {
		if (replicasToRemove.size() == 0) {
			...
			/*
			 * Determine entries to remove. These entries are located between the id of the
			 * local peer and the argument sendingNodeID
			 */
	...
	public final Set<Entry> retrieveEntries(ID id) throws CommunicationException {
		...
		if (this.references.getPredecessor() != null
				&& (!id.isInInterval(this.references.getPredecessor().getNodeID(), this.nodeID)
						&& !id.equals(this.nodeID))) { // Modified by Daniel Shen, for the case that the key is equal to
														// the local ID
