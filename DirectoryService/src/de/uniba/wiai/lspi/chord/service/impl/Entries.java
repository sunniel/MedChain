...
final class Entries {
	...
	/**
	 * Local hash table for entries. Is synchronized, st. methods do not have to be
	 * synchronized.
	 */
	...
	final void add(Entry entryToAdd) {
		...
		synchronized (this.entries) {
			// Modified by Daniel Shen, for entry replacement add
			...
			// Modified by Daniel Shen, add entry to DB
//			String id = entryToAdd.getId().toString();
//			EntryDAO dao = new EntryDAO();
//			try {
//				if (dao.getEntry(id) != null) {
//					dao.removeEntry(id);
//				}
//				dao.addEntry(entryToAdd);
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
		}
	...
	final void remove(Entry entryToRemove) {
		...
		synchronized (this.entries) {
			// Modified by Daniel Shen, check whether the entry is in DB
//			EntryDAO dao = new EntryDAO();
//			try {
//				Entry entry = dao.getEntry(entryToRemove.getId().toString());
//				if (entry != null) {
//					// add it to cache
//					Set<Entry> values = new HashSet<Entry>();
//					this.entries.put(entryToRemove.getId(), values);
//					values.add(entryToRemove);
//				}
//			} catch (Exception e) {
...
//			}
			...
			if (this.entries.containsKey(entryToRemove.getId())) {
				// Modified by Daniel Shen, remove the entire entries with the given ID
				...
				// Modified by Daniel Shen; Remove entry from DB
				// EntryDAO dao = new EntryDAO();
//				try {
//					dao.removeEntry(entryToRemove.getId().toString());
//				} catch (Exception e) {
//					// TODO Auto-generated catch block
//					e.printStackTrace();
//				}
	...
	/**
	 * Returns a set of entries matching the given ID. If no entries match the given
	 * ID, an empty set is returned.
	 * 
	 * @param id
	...
	final Set<Entry> getEntries(ID id) {
		...
		synchronized (this.entries) {
			/*
			 * This has to be synchronized as the test if the map contains a set associated
			 * with id can succeed and then the thread may hand control over to another
			 * thread that removes the Set belonging to id. In that case
			 * this.entries.get(id) would return null which would break the contract of this
			 * method.
			 */
			if (this.entries.containsKey(id)) {
				Set<Entry> entriesForID = this.entries.get(id);
				/*
				 * Return a copy of the set to avoid modification of Set stored in this.entries
				 * from outside this class. (Avoids also modifications concurrent to iteration
				 * over the Set by a client of this class.
				 */
				if (debugEnabled) {
...
//			else {
//				// Modified by Daniel Shen; Try to retrieve entry from DB
//				EntryDAO dao = new EntryDAO();
//				try {
//					Entry entry = dao.getEntry(id.toString());
//					if (entry != null) {
//						// add it to cache
//						this.add(entry);
//						Set<Entry> entriesForID = new HashSet<Entry>();
//						entriesForID.add(entry);
//						return entriesForID;
//					}
//				} catch (Exception e) {
//					// TODO Auto-generated catch block
...
//			}
	...
	/**
	 * Returns all entries in interval, excluding lower bound, but including upper
	 * bound
	 * 
	 * @param fromID
	 *            Lower bound of IDs; entries matching this ID are NOT included in
	 *            result.
	 * @param toID
