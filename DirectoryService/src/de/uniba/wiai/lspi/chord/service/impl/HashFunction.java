...
final class HashFunction {
	 ...
	 *             If there is an internal problem with the specified hash function
	 *             of the Java API.
	 */
	...
	private final ID createIDByURL(byte[] testBytes) {
		/*
		 * synchronize access to messageDigest, so that messageDigest cannot be reset by
		 * another thread before calculating digest.
		 */
		return new ID(new DigestUtils(MessageDigestAlgorithms.SHA_256).digest(testBytes));
	}
	...
	final ID createUniqueNodeID(URL incomingURL) {
		...
		// Modified by Daniel Shen
		ID resultKey = this.createIDByURL(id.getBytes());
		return resultKey;
	}
