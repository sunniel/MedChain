...
public final class SocketProxy extends Proxy implements Runnable {
	...
	/**
	 * The {@link URL}of the node that uses this proxy to connect to the node, which
	 * is represented by this proxy.
	 * 
	 */
	...
	/**
	 * The socket that provides the connection to the node that this is the Proxy
	 * for. This is transient as a proxy can be transferred over the network. After
	 * transfer this socket has to be restored by reconnecting to the node.
	 */
	private transient Socket mySocket;

	/**
	 * The {@link ObjectOutputStream}this Proxy writes objects to. This is transient
	 * as a proxy can be transferred over the network. After transfer this stream
	 * has to be restored.
	 */
	private transient ObjectOutputStream out;

	/**
	 * The {@link ObjectInputStream}this Proxy reads objects from. This is transient
	 * as a proxy can be transferred over the network. After transfer this stream
	 * has to be restored.
	 */
	 ...
	 * transient as a proxy can be transferred over the network. After transfer this
	 * stream has to be restored.
	 */
	private transient Map<String, Response> responses;

	/**
	 * {@link Map} where threads are put in that are waiting for a repsonse. Key:
	 * identifier of the request (same as for the response). Value: The Thread
	 * itself.
	 */
	private transient Map<String, WaitingThread> waitingThreads;

	/**
	 * This indicates that an exception occured while waiting for responses and that
	 * the connection to the {@link Node node}, that this is the proxy for, could
	...
	public static SocketProxy create(URL urlOfLocalNode, URL url) throws CommunicationException {
		synchronized (proxies) {
			/*
			 * added on 21.03.2006 by sven. See documentation of method createProxyKey(URL,
			 * URL);
			 */
	...
	static void shutDownAll() {
		// TODO (Daniel Shen) for bug fixing
		...
		// Copy retired set to iterate over new collection
		// that isn't modified during the iteration
		synchronized (proxies) {
			Set<String> keys = new HashSet<String>(proxies.keySet());
			for (String key : keys) {
				proxies.get(key).disconnect();
			}
			proxies.clear();
	 ...
	 * {@link #proxies} with help of their remote URL as key, so that they were a
	 * kind of singleton for that URL. But the key has to consist of the URL of the
	 * local peer, that uses the proxy, and the remote URL as SocketProxies must
	 * only be (kind of) a singleton per local and remote URL.
	 * 
	 * @param localURL
	 * @param remoteURL
	...
	/**
	 * Private method to send requests over the socket. This method is synchronized
	 * to ensure that no other thread concurrently accesses the
	...
	/**
	 * Called in a method that is delegated to the {@link Node node}, that this is
	 * the proxy for. This method blocks the thread that calls the particular method
	...
	private Response waitForResponse(Request request) throws CommunicationException {
		...
		logger.debug("Trying to wait for response with identifier " + responseIdentifier + " for method "
		...
		synchronized (this.responses) {
			...
			/*
			 * Test if response is already available (Maybe response arrived before we
			 * reached this point).
			 */
			...
			logger.debug("Response for request with identifier " + responseIdentifier + " for method "
					+ MethodConstants.getMethodName(request.getRequestType()) + " received.");
	...
	/**
	 * This method is called by {@link #run()}when it receives a {@link Response}.
	 * The {@link Thread thread}waiting for response is woken up and the response is
	...
	/**
	 * Creates a request for the method identified by <code>methodIdentifier</code>
	 * with the parameters <code>parameters</code>. Sets also field
	...
	private Request createRequest(int methodIdentifier, Serializable[] parameters) {
		if (logger.isEnabledFor(DEBUG)) {
			logger.debug("Creating request for method " + MethodConstants.getMethodName(methodIdentifier)
	...
	public List<Node> notify(Node potentialPredecessor) throws CommunicationException {
		...
		RemoteNodeInfo nodeInfoToSend = new RemoteNodeInfo(potentialPredecessor.getNodeURL(),
	...
	/**
	 * This method has to be called at first in every method that uses the socket to
	 * connect to the node this is the proxy for. This method establishes the
	 * connection if not already done. This method has to be called as this proxy
	 * can be serialized and the reference to the socket is transient. So by calling
	 * this method after a transfer the connection to the node is reestablished. The
	 * same applies for {@link #logger}and {@link #responses}.
	 * 
	 * @throws CommunicationException
	 */
	private void makeSocketAvailable() throws CommunicationException {
		if (this.disconnected) {
			throw new CommunicationException(
					"Connection from " + this.urlOfLocalNode + " to remote host " + this.nodeURL + " is broken down. ");
		}
		...
		if (this.mySocket == null) {
			try {
				...
				// modified by daniel shen, for a large but limmited timeout
				...
				try {
					...
					this.mySocket.setSoTimeout(0);
					if (timedOut) {
						System.out.println("Connection to remote host timed out!");
				...
				throw new CommunicationException("Could not set up IO channel " + "to host " + this.nodeURL.getHost(),
						ioe);
			}
		}
	...
	/**
	 * Finalization ensures that the socket is closed if this proxy is not needed
	 * anymore.
	 * 
	 * @throws Throwable
	 */
	 ...
	 * {@link de.uniba.wiai.lspi.chord.com.socket.Response} made by this proxy and
	 * puts them into a datastructure from where the can be collected by the
	 * associated method call that made a
	 * {@link de.uniba.wiai.lspi.chord.com.socket.Request} to the {@link Node}, that
	 * this is the proxy for.
	 */
	public void run() {
		while (!this.disconnected) {
			try {
				...
				logger.fatal("ClassNotFoundException occured during deserialization "
						+ "of response. There is something seriously wrong " + " here! ", cnfe);
			} catch (IOException e) {
				if (!this.disconnected) {
	...
	/**
	 * The string representation of this proxy. Created when {@link #toString()} is
	...
	private static class WaitingThread {
		...
		/**
		 * Returns <code>true</code> when the thread has been woken up by invoking
		 * {@link #wakeUp()}
		 * 
		 * @return
		 */
