...
class InvocationThread implements Runnable {
	...
	/**
	 * Name of property which defines the maximum number of threads in pool created
	 * by {@link #createInvocationThreadPool()}.
	 */
	 ...
	 * {@link #createInvocationThreadPool()} can stay idle before being terminated.
	 */
	 ...
	 * {@link #createInvocationThreadPool()}.
	 */
	private static final int CORE_POOL_SIZE = Integer.parseInt(AppContext.getValue(CORE_POOL_SIZE_PROPERTY_NAME));

	/**
	 * The maximum number of threads in ThreadPool created by
	 * {@link #createInvocationThreadPool()}.
	 */
	private static final int MAX_POOL_SIZE = Integer.parseInt(AppContext.getValue(MAX_POOL_SIZE_PROPERTY_NAME));

	/**
	 * The time threads in ThreadPool created by
	 * {@link #createInvocationThreadPool()} can be idle before being terminated.
	 */
	private static final int KEEP_ALIVE_TIME = Integer.parseInt(AppContext.getValue(KEEP_ALIVE_TIME_PROPERTY_NAME));
	...
	/**
	 * The request that has to be handled by this InvocationThread. Represents the
	...
	public void run() {
		...
		try {
			...
			this.handler.sendFailureResponse(t,
					"Could not execute request! " + "Reason unknown! Maybe this helps: " + t.getMessage(),
					this.request);
		}
		// this.request = null;
		this.handler = null;
	...
	/**
	 * Creates a ThreadPool that is used by the {@link SocketEndpoint} to execute
	 * instances of this class.
	 * 
	 * @return A ThreadPool that is used by the {@link SocketEndpoint} to execute
	 *         instances of this class.
	 */
