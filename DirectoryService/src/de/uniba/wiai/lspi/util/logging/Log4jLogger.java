		...
		if (!configured) {
			...
			try {
				configURL = Thread.currentThread().getContextClassLoader()
						.getResource(AppContext.getValue(PROPERTIES_FILE_PROPERTY));
				if (configURL == null) {
					java.io.File f = new java.io.File(AppContext.getValue(PROPERTIES_FILE_PROPERTY));
					usefile = f.exists();
					configURL = f.toURL();
				} else {
					usefile = true;
			...
			if (usefile) {
				System.out.println("[" + Thread.currentThread().getName() + "] " + "INFO " + Log4jLogger.class.getName()
						+ " - Configuring log4j with '" + AppContext.getValue(PROPERTIES_FILE_PROPERTY) + "'.");
				try {
					if (AppContext.getValue(PROPERTIES_FILE_PROPERTY).toLowerCase().endsWith(".xml")) {
					...
					System.out.println("[" + Thread.currentThread().getName() + "] " + "INFO "
							...
							+ AppContext.getValue(PROPERTIES_FILE_PROPERTY) + "'.");
